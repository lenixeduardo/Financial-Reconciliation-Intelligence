...
def apply_template(template_id: str, payload: TemplateApplyRequest) -> TemplateApplication:
    ...
    left_required = [column for column in template.left_mapping.model_dump().values() if column]
    right_required = [column for column in template.right_mapping.model_dump().values() if column]
